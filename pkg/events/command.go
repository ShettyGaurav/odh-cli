...
const (
	...
	namespaceDiscoveryTimeout = 60 * time.Second
...
func (c *Command) discoverNamespaces(ctx context.Context) error {
	...
	// Fetch DSCI once, extract both namespaces
	dsci, err := client.GetDSCInitialization(ctx, c.Client)
	if err != nil && !apierrors.IsNotFound(err) {
		return clierrors.ErrDSCIFailed(err)
	}

	if dsci != nil {
		c.ApplicationsNS, _ = jq.Query[string](dsci, ".spec.applicationsNamespace")

		monNS, _ := jq.Query[string](dsci, ".spec.monitoring.namespace")
		if monNS != "" {
			c.MonitoringNS = monNS
		} else {
			c.MonitoringNS = c.ApplicationsNS
		}
	}
